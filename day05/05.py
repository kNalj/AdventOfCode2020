...
with open("input05") as file:
    for seat_id in file:
        row_id, column_id = seat_id[:7], seat_id[7:]

        row = decode_row(row_id)
        column = decode_seat(column_id)
        seats.append(int(row) * 8 + int(column))
        if int(row) * 8 + int(column) > max_id:
            max_id = int(row) * 8 + int(column)

    print(max_id)

    for i, val in enumerate(sorted(seats)):
        if sorted(seats)[i+1] != (val + 1):
            print(val+1)
            break
