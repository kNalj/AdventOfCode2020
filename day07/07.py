...
with open("input07") as file:
    for rule in file:
        arr = rule.strip("\n").split(", ")
        container = arr[0].split(" ")[0] + " " + arr[0].split(" ")[1]
        rule_dict[container] = {}
        for rule in arr:
            amount = None
            words = rule.split(" ")
            for word in words:
                if word.isdigit():
                    amount = int(word)
                    color = words[-3] + " " + words[-2]
                    rule_dict[container][color] = amount
                    break

            if amount is None:
                rule_dict[container] = {None}
...
while not_end:
    for outter_bag, bags in rule_dict.items():
        for bag in bags:
            if bag in bag_list:
                lvl.append(outter_bag)
    bag_list = lvl
    if not lvl:
        not_end = False
    else:
...
# TOTAL INSIDE bags of shiny gold		 1469


# part 2
def count_subbags(bag, data):
    if data[bag] == {None}:
        return 1
    total = 0
    for subbag in data[bag]:
        total += data[bag][subbag] * count_subbags(subbag, data)
    return 1 + total


print(count_subbags("shiny gold", rule_dict) - 1)